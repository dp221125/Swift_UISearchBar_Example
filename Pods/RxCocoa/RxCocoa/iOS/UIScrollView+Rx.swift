...
extension Reactive where Base: UIScrollView {
    ...
    public var willBeginDecelerating: ControlEvent<Void> {
        let source = self.delegate.methodInvoked(#selector(UIScrollViewDelegate.scrollViewWillBeginDecelerating(_:))).map { _ in }
        return ControlEvent(events: source)
    }

    /// Reactive wrapper for delegate method `scrollViewDidEndDecelerating`
    public var didEndDecelerating: ControlEvent<Void> {
        let source = self.delegate.methodInvoked(#selector(UIScrollViewDelegate.scrollViewDidEndDecelerating(_:))).map { _ in }
        return ControlEvent(events: source)
    }

    /// Reactive wrapper for delegate method `scrollViewWillBeginDragging`
    public var willBeginDragging: ControlEvent<Void> {
        let source = self.delegate.methodInvoked(#selector(UIScrollViewDelegate.scrollViewWillBeginDragging(_:))).map { _ in }
        return ControlEvent(events: source)
    }
    ...
    public var willEndDragging: ControlEvent<WillEndDraggingEvent> {
        let source = self.delegate.methodInvoked(#selector(UIScrollViewDelegate.scrollViewWillEndDragging(_:withVelocity:targetContentOffset:)))
    ...
    public var didEndDragging: ControlEvent<Bool> {
        let source = self.delegate.methodInvoked(#selector(UIScrollViewDelegate.scrollViewDidEndDragging(_:willDecelerate:))).map { value -> Bool in
    ...
    public var didZoom: ControlEvent<Void> {
        let source = self.delegate.methodInvoked(#selector(UIScrollViewDelegate.scrollViewDidZoom)).map { _ in }
        return ControlEvent(events: source)
    }

    /// Reactive wrapper for delegate method `scrollViewDidScrollToTop`
    public var didScrollToTop: ControlEvent<Void> {
        let source = self.delegate.methodInvoked(#selector(UIScrollViewDelegate.scrollViewDidScrollToTop(_:))).map { _ in }
        return ControlEvent(events: source)
    }
    ...
    public var didEndScrollingAnimation: ControlEvent<Void> {
        let source = self.delegate.methodInvoked(#selector(UIScrollViewDelegate.scrollViewDidEndScrollingAnimation(_:))).map { _ in }
        return ControlEvent(events: source)
    }

    /// Reactive wrapper for delegate method `scrollViewWillBeginZooming(_:with:)`
    public var willBeginZooming: ControlEvent<UIView?> {
        let source = self.delegate.methodInvoked(#selector(UIScrollViewDelegate.scrollViewWillBeginZooming(_:with:))).map { value -> UIView? in
    ...
    public var didEndZooming: ControlEvent<EndZoomEvent> {
        let source = self.delegate.methodInvoked(#selector(UIScrollViewDelegate.scrollViewDidEndZooming(_:with:atScale:))).map { value -> EndZoomEvent in
