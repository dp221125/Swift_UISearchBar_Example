...
extension Reactive where Base: UICollectionView {
    ...
    public func modelDeselected<T>(_: T.Type) -> ControlEvent<T> {
        let source: Observable<T> = self.itemDeselected.flatMap { [weak view = self.base as UICollectionView] indexPath -> Observable<T> in
            guard let view = view else {
                return Observable.empty()
            }
...
extension Reactive where Base: UICollectionView {
    ...
    public var cancelPrefetchingForItems: ControlEvent<[IndexPath]> {
        let source = self.prefetchDataSource.methodInvoked(#selector(UICollectionViewDataSourcePrefetching.collectionView(_:cancelPrefetchingForItemsAt:)))
            .map { a in
                try castOrThrow([IndexPath].self, a[1])
            }
